...
var expectedTrackedCommands = []string{
	...
	// Pipelines are GA, so unlike the workload/artifact subtrees below there is
	// no feature gate hiding them from the live RootCmd and they need no
	// separate standalone list.
	"dr pipeline create",
	"dr pipeline get",
	"dr pipeline clone",
	"dr pipeline list",
