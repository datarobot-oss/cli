...
func writeContents(contents, dotenvFile, templateFile string) error {
	...
	for _, line := range lines {
		...
		if !strippedHeader && (strings.HasPrefix(trimmed, "# Autogenerated by dr dotenv") ||
			strings.HasPrefix(trimmed, "# Edited using `dr dotenv`")) {
			continue
		}
	...
	var line string
	if templateFile == "" {
		line = "# Edited using `dr dotenv` on " + timestamp + "\n\n"
	} else {
		line = "# Edited using `dr dotenv` from " + templateFile + " on " + timestamp + "\n\n"
	}
