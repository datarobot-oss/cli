...
- **`Taskfile.gen.yaml`**&mdash;automatically generated and used by `dr run` / `dr task run`.
- **`Taskfile.yaml`**&mdash;generated by `dr task compose` for use with the native `task` command or for manual inspection.
...
- [Template system](README.md)&mdash;template system overview and documentation.
