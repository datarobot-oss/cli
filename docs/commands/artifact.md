...
- `sync` computes a three-way diff against the last synced state and applies it in one versioned step. Conflicts resolve to the remote copy, and your version is kept as a `*.LOCAL.<timestamp>` file. Preview with `--dry-run`, or use `--diff` to also see per-file diffs. Both exit before any remote write. The upload requests `overwrite=REPLACE` on the multipart form, so re-syncing an existing artifact replaces the prior version instead of the server renaming the upload to `name (2).zip`.
- `--verify` forces a remote round-trip before planning even when the artifact has not drifted, so BASE-vs-REMOTE differences the fast path cannot see are surfaced as `divergence` entries in the plan and in the plan JSON. During execution the manifest is repaired from the server's real state, and after apply the uploaded paths' checksums are re-fetched and compared against what this run streamed; a mismatch fails the run. `--verify` composes with `--dry-run` and `--diff`.
- Symlinks in the source tree are not followed. Each skipped symlink prints a stderr notice (worded differently for file vs directory symlinks, since a directory symlink omits an entire subtree); the prose is bounded at five paths, with any remainder summarized as "and N more". The plan JSON carries the full list in `skippedSymlinks`, always emitted and `[]` when none. Symlinks excluded by `.drignore` or by the hardcoded system excludes are not reported.
