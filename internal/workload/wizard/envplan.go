...
func (o Options) envPlan(parsed *manifest.Manifest, detected Detected, wanted []manifest.EnvVar) []envAction {
	...
	return oncePerName(append(actions, removeActions(parsed, detected)...))
}

// oncePerName keeps the first row a variable earns and drops the rest.
//
// The table promises a name appears exactly once, and the categories it was
// built from used to overlap: an entry still on the credential placeholder was
// also one whose kind no longer matched .env, and also one .env may have
// dropped, so it arrived under two skip reasons and read as two variables that
// happened to share a name. Now that .env wins outright, those are one act on
// one entry and nothing here should have a second row to drop.
//
// It stays because the guarantee belongs to the reader rather than to whatever
// assembles the rows: a category added later must not be able to break it
// quietly. First rather than last, because the rows are assembled in
// descending order of what the run will actually do.
