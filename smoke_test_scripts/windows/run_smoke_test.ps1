...
# Temporarily relax error handling so that debug output written to stderr by native
# commands does not trigger a NativeCommandError under $ErrorActionPreference = "Stop".
$prevErrorActionPreference = $ErrorActionPreference
$ErrorActionPreference = "Continue"
$debug_output = & { dr --debug auth check 2>&1 } | Out-String
